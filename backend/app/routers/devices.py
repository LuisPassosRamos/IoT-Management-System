...
async def get_devices(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    devices_status = []
    for device_data in devices:
        simulated_status = device_simulator.get_device_status(device_data["id"])
        if simulated_status:
...
async def get_device(device_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
...
async def execute_device_action(
    ...
    # Execute action
    result = device_simulator.execute_device_action(device_id, action_request.action)
