...
async def get_resources(current_user: Dict[str, Any] = Depends(get_current_user)):
...
async def reserve_resource(
    ...
    if not resource["available"]:
        raise HTTPException(status_code=400, detail="Resource is already reserved")
    
    # Update resource availability
    updates = {
        "available": False,
        "reserved_by": reservation_request.user_id
    }
    ...
    if resource.get("device_id"):
        ...
        if device_data and device_data["type"] == "lock":
            ...
            result = device_simulator.execute_device_action(resource["device_id"], "unlock")
            
            if result.get("success"):
                storage.update_device(resource["device_id"], {"status": result.get("status")})
...
async def release_resource(
    ...
    if resource.get("device_id"):
        ...
        if device_data and device_data["type"] == "lock":
            device_simulator.register_device(device_data)
            result = device_simulator.execute_device_action(resource["device_id"], "lock")
